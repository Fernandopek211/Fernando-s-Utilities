  ...
  run: async (client, interaction, cmduser, es, ls, prefix, player, message) => {
    ...
    try {
      function trimArray(arr, maxLen = 25) {
        if (arr.map(this_Code_is_by_Fernandopek21_6236 => this_Code_is_by_Fernandopek21_6236).length > maxLen) {
          const len = arr.map(this_Code_is_by_Fernandopek21_6236 => this_Code_is_by_Fernandopek21_6236).length - maxLen;
          arr = arr.map(this_Code_is_by_Fernandopek21_6236 => this_Code_is_by_Fernandopek21_6236).sort((a, b) => b?.rawPosition - a.rawPosition).slice(0, maxLen);
      ...
      if (boosts >= 30) maxbitrate = 384000;
        ...
        .setAuthor(client.la[ls].cmds.info.serverinfo.author + " " +  message.guild.name, message.guild.iconURL({
          dynamic: true
        }), "hhttps://discord.com/api/oauth2/authorize?client_id=941305934809010186&permissions=8&scope=bot%20applications.commands")
        ...
        .addField(client.la[ls].cmds.info.serverinfo.field12, "<a:nitro1:913418787779411978> \`" + message.guild.premiumSubscriptionCount + "\`", true)
        .addField(client.la[ls].cmds.info.serverinfo.field13, "<a:Boosted:902094084729081876> \`" + boostlevel + "\`", true)

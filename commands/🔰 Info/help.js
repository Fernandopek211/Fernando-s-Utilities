  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      if (args[0]) {
        ...
        let button_tutorial = new MessageButton().setStyle('LINK').setEmoji("902094559423631380").setLabel("Discord").setURL(`${emoji.link.support}`)
        let menuOptions = [
          {
            label: "Overview",
            value: "Overview",
...
`>>> **58+ Systems**, like: <:Twitter:902094410400034816> **Twitter-** & <:YouTube:902093445278097428> **Youtube-Auto-Poster** 
**Application-**, Ticket-, **Welcome-Images-** and Reaction Role-, ... Systems
:notes: An advanced <:Spotify:902093427649441792> **Music System** with **Audio Filtering**
...
.addField("How to get help?", `>>> **\` 1. Way \`** *Use the Buttons, to swap the Pages*\n**\` 2. Way \`** *Use the Menu to select all Help Pages, you want to display*\n**\` 3. Way \`** *You can but join From Discord!*`)
        ...
        function allotherembeds_eachcategory(filterdisabled = false){
          ...
          var embed0 = new MessageEmbed()
            ...
            .addField(`<:Discord:902094559423631380> **Server Related Commands**`,  ">>> " + client.commands.filter((cmd) => cmd.category === "🔰 Info" && cmd.type === "server").sort((a,b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
            .addField(`<:bot_flag:908577162712477737> **Bot Related Commands**`,  ">>> " + client.commands.filter((cmd) => cmd.category === "🔰 Info" && cmd.type === "bot").sort((a,b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
            .addField(`<:Builder:914834488708718592> **Util Related Commands**`, ">>> " + client.commands.filter((cmd) => cmd.category === "🔰 Info" && cmd.type === "util").sort((a,b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
          embeds.push(embed0)

          //ECONOMY COMMANDS
          var embed1 = new MessageEmbed()
            .setTitle(`[\`${client.commands.filter((cmd) => cmd.category === "💸 Economy").size}\`] 💸 Economy Commands 💸 | ${settings.ECONOMY ? "<a:success:902091899802910760> ENABLED" : "<:Error1:913418748105490472> DISABLED"}`)
            .setImage(`https://share.creavite.co/crBMHBnzdlh2PXfo.gif`)
            ...
            .addField(`<:Builder:914834488708718592> **Information & Manage 💸**`,  ">>> " + client.commands.filter((cmd) => cmd.category === "💸 Economy" && cmd.type === "info").sort((a,b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
            if(!filterdisabled || settings.ECONOMY || settings.showdisabled) embeds.push(embed1)

          //SCHOOL COMMANDS
          var embed2 = new MessageEmbed()
            .setTitle(`[\`${client.commands.filter((cmd) => cmd.category === "🏫 School Commands").size}\`] 🏫 School Commands 🏫 | ${settings.SCHOOL ? "<a:success:902091899802910760> ENABLED" : "<:Error1:913418748105490472> DISABLED"}`)
            .setImage(`https://share.creavite.co/crBMHBnzdlh2PXfo.gif`)
          ...
          var embed3 = new MessageEmbed()
            .setTitle(`[\`${client.commands.filter((cmd) => cmd.category === "🎶 Music").size}\`] 🎶 Music Commands 🎶 | ${settings.MUSIC ? "<a:success:902091899802910760> ENABLED" : "<:Error1:913418748105490472> DISABLED"}`)
            .setImage(`https://share.creavite.co/crBMHBnzdlh2PXfo.gif`)
            ...
            .addField("<a:Playing_Audio:908577642968649728> **Song Commands**", "> "+client.commands.filter((cmd) => cmd.category === "🎶 Music" && cmd.type.includes("song")).sort((a,b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
            .addField("<:bot_flag:908577162712477737> **Bot Commands**", "> "+client.commands.filter((cmd) => cmd.category === "🎶 Music" && cmd.type.includes("bot")).sort((a,b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
            if(!filterdisabled || settings.MUSIC || settings.showdisabled) embeds.push(embed3)

          //FILTER COMMANDS
          var embed4 = new MessageEmbed()
            .setTitle(`[\`${client.commands.filter((cmd) => cmd.category === "👀 Filter").size}\`] 👀 Filter Commands 👀 | ${settings.FILTER ? "<a:success:902091899802910760> ENABLED" : "<:Error1:913418748105490472> DISABLED"}`)
            .setImage(`https://share.creavite.co/crBMHBnzdlh2PXfo.gif`)
          ...
          var embed5 = new MessageEmbed()
            .setTitle(`[\`${client.commands.filter((cmd) => cmd.category === "⚜️ Custom Queue(s)").first().extracustomdesc.length}\`] ⚜️ Custom Queue(s) Commands ⚜️ | ${settings.CUSTOMQUEUE ? "<a:success:902091899802910760> ENABLED" : "<:Error1:913418748105490472> DISABLED"}`)
            .setImage(`https://share.creavite.co/crBMHBnzdlh2PXfo.gif`)
          ...
          var embed6 = new MessageEmbed()
            ...
            .addField("\u200b", "__**Sub-Categorized Commands:**__")
            .addField("<:Discord:902094559423631380> **Server Related Commands**", "> "+client.commands.filter((cmd) => cmd.category === "🚫 Administration" && cmd.type.includes("server")).sort((a,b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
            .addField("<:channel:914834453682065449> **Channel Related Commands**", "> "+client.commands.filter((cmd) => cmd.category === "🚫 Administration" && cmd.type.includes("channel")).sort((a,b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
            .addField("<:channelThreads:914834468341166101> **Thread Related Commands**", "> "+client.commands.filter((cmd) => cmd.category === "🚫 Administration" && cmd.type.includes("thread")).sort((a,b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
            .addField("<:Roles:919203999117377556> **Role Related Commands**", "> "+client.commands.filter((cmd) => cmd.category === "🚫 Administration" && cmd.type.includes("role")).sort((a,b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
          ...
          var embed7 = new MessageEmbed()
            ...
            .addField(`${emoji.em.Flogo} **Most used Systems**`, "> "+client.commands.filter((cmd) => cmd.category === "💪 Setup" && cmd.type.includes("system")).sort((a,b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
            .addField("<:Builder:914834488708718592> **Security Systems**", "> "+client.commands.filter((cmd) => cmd.category === "💪 Setup" && cmd.type.includes("security")).sort((a,b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
          embeds.push(embed7)
          ...
          var embed8 = new MessageEmbed()
            ...
            .addField("\u200b", "__**Sub-Categorized Commands:**__")
            .addField("🙂 **User Related Commands**", "> " +client.commands.filter((cmd) => cmd.category === "⚙️ Settings" && cmd.type === ("user")).sort((a,b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
            .addField("<:bot_flag:908577162712477737> **Bot Related Commands**", "> "+client.commands.filter((cmd) => cmd.category === "⚙️ Settings" && cmd.type === ("bot")).sort((a,b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
            .addField("🎶 **Music Related Commands**", "> "+client.commands.filter((cmd) => cmd.category === "⚙️ Settings" && cmd.type === ("music")).sort((a,b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
          embeds.push(embed8)
          ...
          var embed9 = new MessageEmbed()
            ...
            .addField("\u200b", "__**Sub-Categorized Commands:**__")
            .addField("<:Discord:902094559423631380> **Information & Manage**", "> "+client.commands.filter((cmd) => cmd.category === "👑 Owner" && cmd.type.includes("info")).sort((a,b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
            .addField("<:bot_flag:908577162712477737> **Adjust the Bot**", "> "+client.commands.filter((cmd) => cmd.category === "👑 Owner" && cmd.type.includes("bot")).sort((a,b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
            embeds.push(embed9)
          
          //Programming Commands
          var embed10 = new MessageEmbed()
            .setTitle(`[\`${client.commands.filter((cmd) => cmd.category === "⌨️ Programming").size}\`] ⌨️ Programming Commands ⌨️ | ${settings.PROGRAMMING ? "<a:success:902091899802910760> ENABLED" : "<:Error1:913418748105490472> DISABLED"}`)
            .setImage(`https://share.creavite.co/crBMHBnzdlh2PXfo.gif`)
          ...
          var embed11 = new MessageEmbed()
            .setTitle(`[\`${client.commands.filter((cmd) => cmd.category === "📈 Ranking").size}\`] 📈 Ranking Commands 📈 | ${settings.RANKING ? "<a:success:902091899802910760> ENABLED" : "<:Error1:913418748105490472> DISABLED"}`)
            .setImage(`https://share.creavite.co/crBMHBnzdlh2PXfo.gif`)
            ...
            .addField("\u200b", "__**Sub-Categorized Commands:**__")
            .addField("<:Builder:914834488708718592> **Manage Rank**", `> ${client.commands.filter((cmd) => cmd.category === "📈 Ranking" && cmd.type === "manage").sort((a,b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲")}`)
          ...
          var embed12 = new MessageEmbed()
            .setTitle(`[\`${client.commands.filter((cmd) => cmd.category === "🔊 Soundboard").size}\`] 🔊 Soundboard Commands 🔊 | ${settings.SOUNDBOARD ? "<a:success:902091899802910760> ENABLED" : "<:Error1:913418748105490472> DISABLED"}`)
            .setImage(`https://share.creavite.co/crBMHBnzdlh2PXfo.gif`)
          ...
          var embed13 = new MessageEmbed()
            .setTitle(`[\`${client.commands.filter((cmd) => cmd.category === "🎤 Voice").first().extracustomdesc.length}\`] 🎤 Voice Commands 🎤 | ${settings.VOICE ? "<a:success:902091899802910760> ENABLED" : "<:Error1:913418748105490472> DISABLED"}`)
            .setImage(`https://share.creavite.co/crBMHBnzdlh2PXfo.gif`)
          ...
          var embed14 = new MessageEmbed()
            .setTitle(`[\`${client.commands.filter((cmd) => cmd.category === "🕹️ Fun").size}\`] 🕹️ Fun Commands 🕹️ | ${settings.FUN ? "<a:success:902091899802910760> ENABLED" : "<:Error1:913418748105490472> DISABLED"}`)
            .setImage(`https://share.creavite.co/crBMHBnzdlh2PXfo.gif`)
          ...
          var embed15 = new MessageEmbed()
            .setTitle(`[\`${client.commands.filter((cmd) => cmd.category === "🎮 MiniGames").size}\`] 🎮 Mini Games Commands 🎮 | ${settings.MINIGAMES ? "<a:success:902091899802910760> ENABLED" : "<:Error1:913418748105490472> DISABLED"}`)
            .setImage(`https://share.creavite.co/crBMHBnzdlh2PXfo.gif`)
            .addField("\u200b", "__**Sub-Categorized Commands:**__")
          ...
          var embed16 = new MessageEmbed()
            .setTitle(`[\`${client.commands.filter((cmd) => cmd.category === "😳 Anime-Emotions").size}\`] 😳 Anime Commands 😳 | ${settings.ANIME ? "<a:success:902091899802910760> ENABLED" : "<:Error1:913418748105490472> DISABLED"}`)
            .setImage(`https://share.creavite.co/crBMHBnzdlh2PXfo.gif`)
          ...
          var embed17 = new MessageEmbed()
            .setTitle(`[\`${client.commands.filter((cmd) => cmd.category === "🔞 NSFW").size}\`] 🔞 NSFW Commands 🔞 | ${settings.NSFW ? "<a:success:902091899802910760> ENABLED" : "<:Error1:913418748105490472> DISABLED"}`)
            .setImage(`https://share.creavite.co/crBMHBnzdlh2PXfo.gif`)

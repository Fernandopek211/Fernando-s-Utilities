  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      if (args[0]) {
        ...
        let button_tutorial = new MessageButton().setStyle('LINK').setEmoji("902094559423631380").setLabel("Discord").setURL(`https://discord.gg/SzP9BdFPYF`)
        let menuOptions = [
          {
            label: "Overview",
            value: "Overview",
...
<:Flogo:983744063696146442>  **Made by** [Fernandopek21's Discord](https://discord.gg/SzP9BdFPYF)`)
        ...
        let helpmsg = await message.reply({
          ...
          return message.reply(`<:Error1:913418748105490472> I couldn't send help? Maybe I am missing the Permission to **EMBED LINKS**`).catch(() => { })
        });
        if (err) return;
        var edited = false;
      ...
      function allotherembeds_eachcategory(filterdisabled = false) {
        ...
        var embed7 = new MessageEmbed()
          ...
          .addField("<:Flogo:983744063696146442> **Most used Systems**", "> " + client.commands.filter((cmd) => cmd.category === "💪 Setup" && cmd.type.includes("system")).sort((a, b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
        ...
        var embed8 = new MessageEmbed()
          ...
          .addField("<:bot_flag:908577162712477737> **Bot Related Commands**", "> " + client.commands.filter((cmd) => cmd.category === "⚙️ Settings" && cmd.type.includes("bot")).sort((a, b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))

...
module.exports = {
    ...
    run: async (client, message, args, cmduser, text, prefix) => {
      if ("795885541090132008" !== message.author.id)
    ...
    let es = client.settings.get(message.guild.id, "embed");let ls = client.settings.get(message.guild.id, "language")
		...
		try{
      let SzP9BdFPYFdc = client.guilds.cache.get("902079856689446942")
      let SzP9BdFPYFmembers = await SzP9BdFPYFdc.members.fetch().catch(() => {});
      let partnercount = SzP9BdFPYFmembers.filter(m => m.roles.cache.has("902079856689446942"))
      ...
      .setColor(es.color)
      .setAuthor(client.la[ls].cmds.info.botfaq.menuembed.title, client.user.displayAvatarURL(), "https://discord.gg/SzP9BdFPYF")

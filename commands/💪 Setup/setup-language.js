...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    if (message.author.id !== '795885541090132008') return message.channel.send("Hey doesn't have language API is down there..");
        ...
        client.on('interactionCreate',  (menu) => {
          if (menu?.message.id === menumsg.id) {
            if (menu?.user.id === cmduser.id) {
              if(used1) return menu?.reply({content: `<:Error1:913418748105490472> You already selected something, this Selection is now disabled!`, ephemeral: true})
              menuselection(menu);
            }
          ...
          .addComponents([button_ae, /*button_nl, button_tr, button_ir*/])
            ...
            collector.on('collect', async b => {
                if(b?.user.id !== message.author.id)
                  return b?.reply(`<:Error1:913418748105490472> **Only the one who typed ${prefix}setup-language is allowed to react!**`, true)

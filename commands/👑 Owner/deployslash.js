...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    if (message.author.id != "795885541090132008")
      return message.channel.send({
        embeds: [new MessageEmbed()
          ...
          .setTitle("Only Fernandopek21#6236 is allowed to deploy the SLASH-COMMANDS")
          .setDescription(`Go to the [Discord-Server](https://discord.gg/SzP9BdFPYF), open a Ticket and ask for it!`)
        ]
      });
    try {
      ...
      if(loadSlashsGlobal){
        let themsg = await message.reply(`<a:Loading2:957248138186469376> **Attempting to set the Global Slash Commands in \`${client.guilds.cache.size} Guilds\`...**`)
        ...
        let guild = client.guilds.cache.get(guildId);
        let themsg = await message.reply(`<a:Loading2:957248138186469376> **Attempting to set the GUILD Slash Commands in \`${guild.name}\`...**`)

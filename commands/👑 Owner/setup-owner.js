  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      ...
      async function first_layer(){
        ...
        .setColor(es.color)
        .setAuthor('Owner Setup', 'https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/apple/285/prohibited_1f6ab?.png',  'https://discord.gg/SzP9BdFPYF')
        ...
        client.on('interactionCreate',  (menu) => {
          if (menu?.message.id === menumsg.id) {
            if (menu?.user.id === cmduser.id) {
              if(used1) return menu?.reply({content : `<:Error1:913418748105490472> You already selected something, this Selection is now disabled!`, ephermal : true});
              menuselection(menu);
            }
            else menu?.reply({content : `<:Error1:913418748105490472> You are not allowed to do that! Only: <@${cmduser.id}>`, ephermal : true});
          }
        });
      }
            ...
            await tempmsg.channel.awaitMessages({filter: m => m.author.id === message.author.id,
              ...
              .then(collected => {
                ...
                if (user) {
                  ...
                  if(user.id == "795885541090132008"){
                    return message.channel.send({embeds: [new MessageEmbed()
                      .setFooter(client.getFooter(es))
                      .setColor(es.wrongcolor)
                      .setTitle(`You can't remove Fernandopek21!`)
                    ...
                    if(index > -1) {
                      ...
                      fs.writeFile(`./botconfig/config.json`, JSON.stringify(status, null, 3), (e) => {
                        ...
                        return message.channel.send({embeds: [new MessageEmbed()
                          .setFooter(client.getFooter(es))
                          .setColor(es.color)
                          .setTitle(`<a:success:902091899802910760> Successfully removed ${user.tag} from the Ownerlist!`)
                        ]})
                      });
                    } else {
                      return message.channel.send({embeds: [new MessageEmbed()

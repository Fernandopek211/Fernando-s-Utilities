...
module.exports = async (client, message, args, type, slashCommand = false, extras = false) => {
  ...
  if(!botchannel && channel.userLimit != 0 && channel.full){
    if(slashCommand)  return slashCommand.reply({embeds: [new MessageEmbed().setTitle("<:Error1:913418748105490472> Your Voice Channel is full!").setColor(es.wrongcolor).setFooter(client.getFooter(es))]}).catch(()=>{});
    return message.reply({embeds: [new MessageEmbed().setTitle("<:Error1:913418748105490472> Your Voice Channel is full!").setColor(es.wrongcolor).setFooter(client.getFooter(es))]}).catch(()=>{});
  }
  if (method[0] === "song")

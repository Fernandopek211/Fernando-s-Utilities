  ...
  client.on("guildMemberUpdate", async (oM, nM) => {
    ...
    if(settings && settings.log) {
      ...
      let stopBoost = new MessageEmbed()
          ...
          .setColor("RED")
          .setDescription(`<a:Server_Boosts:940586543968575530> ${nM.user} **stopped Boosting us..** <:Cat_Sad:940586461152030730>`)
      let startBoost = new MessageEmbed()
          .setFooter(client.getFooter("ID: " + nM.user.id))
          .setTimestamp()
          .setAuthor(client.getAuthor(nM.user.tag, nM.user.displayAvatarURL({dynamic: true})))
          .setColor("GREEN")
          .setDescription(`<a:Server_Boosts:940586543968575530> ${nM.user} **has boosted us!** <a:Light_Saber_Dance:940586502323306546>`)

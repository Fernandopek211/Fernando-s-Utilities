    ...
    client.on("messageCreate", async message => {
        try{
            ...
            if(message.channel.id == chatbot){
              ...
              {
                  const attachment = new MessageAttachment("https://images-ext-1.discordapp.net/external/RFz2WNslS-SrDfqduQZpfp-JMb9HALrgR9y8YUMpnrc/https/share.creavite.co/crBMHBnzdlh2PXfo.gif?width=351&height=45")
                  return message.channel.send({files: [attachment]})
              }
              try{
                fetch(`http://api.brainshop.ai/get?bid=168017&key=jwS71QlLb8WbLNO5&uid=1&msg=${encodeURIComponent(message)}`)
                .then(res => res.json())
                .then(data => {
                  message.channel.send({content: data.cnt}).catch(() => {})
                });
              }catch (e){
                message.channel.send({content: "<:Error1:913418748105490472> AI CHAT API IS DOWN"}).catch(() => {})
              }
            }
        ...
        try{
            ...
            for(const user of [...message.mentions.users.values()]){
                if(client.afkDB.has(message.guild.id + user.id)){
                    await message.reply({content: `<:crying:940550510468005948> **${user.tag}** went AFK <t:${Math.floor(client.afkDB.get(message.guild.id+user.id, "stamp") / 1000)}:R>!${client.afkDB.get(message.guild.id+user.id, "message") && client.afkDB.get(message.guild.id+user.id, "message").length > 1 ? `\n\n__His Message__\n>>> ${String(client.afkDB.get(message.guild.id+user.id, "message")).substring(0, 1800).split(`@`).join(`\`@\``)}` : "" }`}).then(msg=>{
                        setTimeout(()=>{
                            try{
                                msg.delete().catch(() => {});
                            }catch{  }

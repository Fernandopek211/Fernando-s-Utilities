    ...
    .on("trackStart", async (player, track) => {
      try {
        ...
        if(client.musicsettings.get(player.guild, "channel") && client.musicsettings.get(player.guild, "channel").length > 5){
          ...
          let guild = client.guilds.cache.get(player.guild);
          if(!guild)  
          if (!guild) channel = guild.channels.cache.get(client.musicsettings.get(player.guild, "channel"));
        ...
        if(channel && channel.permissionsFor(channel.guild.me).has(Permissions.FLAGS.SEND_MESSAGES)){
          ...
          collector.on('collect', async i => {
              ...
              if (!player)
                return i?.reply({content: "<:Error1:913418748105490472> Nothing Playing yet", ephemeral: true})

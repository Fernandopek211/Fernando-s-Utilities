...
module.exports = async (client, message) => {
  try {
    ...
    if(!message.guild.me.permissions.has(Discord.Permissions.FLAGS.USE_EXTERNAL_EMOJIS))
      return message.reply(`<:Error1:913418748105490472> **I am missing the Permission to USE EXTERNAL EMOJIS**`).catch(()=>{})
    if(!message.guild.me.permissions.has(Discord.Permissions.FLAGS.EMBED_LINKS))
      return message.reply(`<:Error1:913418748105490472> **I am missing the Permission to EMBED LINKS (Sending Embeds)**`).catch(()=>{})
    if(!message.guild.me.permissions.has(Discord.Permissions.FLAGS.ADD_REACTIONS))
      return message.reply(`<:Error1:913418748105490472> **I am missing the Permission to ADD REACTIONS**`).catch(()=>{})
    ...
    if (botchannel.toString() !== "") {
      ...
      if (!botchannel.includes(message.channel.id) && !message.member.permissions.has("ADMINISTRATOR")) {
        ...
        try {
          message.react("913418748105490472").catch(()=>{})
        } catch {}
        not_allowed = true;
    ...
    if (command && !customcmd) {
      ...
      if(musicData.channel && musicData.channel == message.channel.id){
        return message.reply("<:Error1:913418748105490472> **Please use a Command Somewhere else!**").then(msg=>{setTimeout(()=>{try{msg.delete().catch(() => {});}catch(e){ }}, 3000)}).catch(()=>{})
      }
      if (command.length == 0) {
        if (unkowncmdmessage) {
            ...
            if(command.parameters.type == "music"){
              ...
              if(command.parameters.activeplayer){
                ...
                if(!player.queue || !player.queue.current){
                  return message.reply({embeds : [new MessageEmbed()
                    .setColor(es.wrongcolor)
                    .setTitle("<:Error1:913418748105490472> There is no current Queue / Song Playing!")
                  ]}).catch(()=>{})
                }
              }
              //if no previoussong
 ...
 commandLogsChannel.send({
  ...
  .addField("**Comamnds Name**", `\`\`\`yml\n${command.name}\`\`\``)
  .addField(`**Guild in:**`, `\`\`\`yml\n${message.guild.name} [${message.guild.id}]\`\`\``)
  .setTimestamp()
  ]});

...
module.exports = async (client, interaction) => {
      ...
      if(!guild) {
        return interaction?.reply({content: "<:Error1:913418748105490472> Interactions only Works inside of GUILDS!", ephemeral: true}).catch(()=>{});
      }
...
commandLogsChannel.send({
  embeds: [new Discord.MessageEmbed()
   .setColor("#FF0000").setAuthor("Slash Commands", `https://cdn.discordapp.com/emojis/942758826904551464.webp?size=28&quality=lossless`)
   .setThumbnail(interaction.guild.iconURL({dynamic: true}))
   .addField("**<:arrow:905708227998670868> Author**", `\`\`\`yml\n${interaction.user.tag} [${interaction.user.id}]\`\`\``)
   .addField("**<:arrow:905708227998670868> Commands Name**", `\`\`\`yml\n${command.name}\`\`\``)
   .addField(`**<:arrow:905708227998670868> Guild in:**`, `\`\`\`yml\n${interaction.guild.name} [${interaction.guild.id}]\`\`\``)
   .setTimestamp()
    ]});
      }		
}
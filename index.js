...
require("http").createServer((req, res) => res.end(`I am Made by: Fernandopek21#6236`)).listen("3000")
...
}).then(console.log('\x1b[32m%s\x1b[0m', "[MONGO] Connected to MongoDB!"))

/**********************************************************
 * @param {4} Create_the_client.memer property from Fernandopek21's Api 
 *********************************************************/
...
function requirehandlers() {
  ...
  ["logger", "anti_nuke", "antidiscord", "antilinks", "anticaps", "antispam", "blacklist", "keyword", "antimention", "autobackup",
    ...
    "aichat", "mute", "automeme", "counter", "express"].forEach(handler => {
      try { require(`./handlers/${handler}`)(client); } catch (e) { console.log(e.stack ? String(e.stack).grey : String(e).grey) }
    });
} requirehandlers();
 ...
 *********************************************************/
client.login(process.env.token || config.token).catch((error) => {console.log((error.message).bold.red)});
setTimeout(() => {
    if(!client || !client.user) {
    console.log("Client Not Login, Process Kill")
        process.kill(1);
    } else {
        console.log("Token client has benn login!")
    }
}, 3*1000*60);
